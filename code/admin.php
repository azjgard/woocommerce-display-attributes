...
function WCDA_enqueue_admin_styles($hook) {
  if ($hook == 'toplevel_page_attribute-manager') {
    wp_register_style(
      'wcda_admin_css', plugins_url('../admin-style.css', __FILE__),
      false, '1.0.0'
    );
    wp_enqueue_style('wcda_admin_css');
  }
}
add_action('admin_enqueue_scripts', 'WCDA_enqueue_admin_styles');
